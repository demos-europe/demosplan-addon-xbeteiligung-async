...
class KommunaleProcedureCreatorTest extends TestCase
{
    ...
    public function testCreateProcedureWithPipeFormatVeranlasserName(): void
    {
        $filePath = 'tests/res/xmlv14/xbeteiligung-test-kommunal.Initiieren.0401.pipe-format.xml';
        $inputMsgXml = file_get_contents(AddonPath::getRootPath($filePath));
        /** @var KommunalInitiieren0401 $inputMsgObj */
        $inputMsgObj = $this->messageParser->getXmlObject($inputMsgXml, '401');
        ...
        // Verify the Veranlasser name contains the pipe format
        $veranlasserName = $inputMsgObj->getNachrichteninhalt()
            ->getBeteiligung()
            ->getAkteurVorhaben()
            ?->getVeranlasser()
            ?->getName()
            ?->getName();
        self::assertStringContainsString('|', $veranlasserName);

        // Act — procedure creation should succeed with pipe-format org name
        $testRoutingKey = 'nrw.cockpit.bap.02.05.00200099.bdp.02.05.00200099.kommunal.initiieren.0401';
        $procedure = $this->sut->createNewKommunalProcedureFromXBeteiligungMessage($inputMsgObj, $testRoutingKey);

        // Assert procedure was created successfully
        self::assertInstanceOf(ProcedureInterface::class, $procedure);
        self::assertSame('TestAdrian33', $procedure->getName());
        self::assertSame('TestAdrian33', $procedure->getExternalName());
    }

    /**
     * Tests the fallback path: full pipe string lookup fails, orgaId-only lookup succeeds.
     */
    public function testCreateProcedureWithPipeFormatFallsBackToOrgaIdPart(): void
    {
        $mockFactory = new MockFactoryTest($this);
        $orgaMock = $mockFactory->getOrgaMock();

        // Build an orgaService mock that rejects the full pipe string but accepts the orgaId part
        $orgaServiceMock = $this->createMock(OrgaServiceInterface::class);
        $orgaServiceMock->method('getOrgaByFields')
            ->willReturnCallback(static function (array $fields) use ($orgaMock): array {
                if (isset($fields['gwId']) && str_contains($fields['gwId'], '|')) {
                    return []; // full pipe string → no match
                }

                return [$orgaMock]; // orgaId-only → match
            });

        $procedureHandlerFactory = new KommunaleProcedureHandlerFactory($mockFactory);
        $sut = $procedureHandlerFactory->createProcedureHandler('creator', $orgaServiceMock);

        // Use the pipe-format XML fixture
        $filePath = 'tests/res/xmlv14/xbeteiligung-test-kommunal.Initiieren.0401.pipe-format.xml';
        $inputMsgXml = file_get_contents(AddonPath::getRootPath($filePath));
        /** @var KommunalInitiieren0401 $inputMsgObj */
        $inputMsgObj = $this->messageParser->getXmlObject($inputMsgXml, '401');
        ...
        $testRoutingKey = 'nrw.cockpit.bap.02.05.00200099.bdp.02.05.00200099.kommunal.initiieren.0401';
        $procedure = $sut->createNewKommunalProcedureFromXBeteiligungMessage($inputMsgObj, $testRoutingKey);

        self::assertInstanceOf(ProcedureInterface::class, $procedure);
        self::assertSame('TestAdrian33', $procedure->getName());
    }

    public function testCreateProcedureWithGwIdFormatVeranlasserName(): void
    {
        $filePath = 'tests/res/xmlv14/xbeteiligung-test-kommunal.Initiieren.0401.gwid-format.xml';
        $inputMsgXml = file_get_contents(AddonPath::getRootPath($filePath));
        /** @var KommunalInitiieren0401 $inputMsgObj */
        $inputMsgObj = $this->messageParser->getXmlObject($inputMsgXml, '401');
        ...
        // Verify the Veranlasser name is a plain gwId (no pipe)
        $veranlasserName = $inputMsgObj->getNachrichteninhalt()
            ->getBeteiligung()
            ->getAkteurVorhaben()
            ?->getVeranlasser()
            ?->getName()
            ?->getName();
        self::assertMatchesRegularExpression('/^[a-z]{2,}-[a-f0-9]{20,}$/i', $veranlasserName);
        self::assertStringNotContainsString('|', $veranlasserName);

        // Act — procedure creation should succeed with gwId org name
        $testRoutingKey = 'nrw.cockpit.bap.02.05.00200099.bdp.02.05.00200099.kommunal.initiieren.0401';
        $procedure = $this->sut->createNewKommunalProcedureFromXBeteiligungMessage($inputMsgObj, $testRoutingKey);
